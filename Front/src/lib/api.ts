...
export const api = {
  ...
  adminProfileCatalogItems: (token: string, params: string) => request<{ items: Profile[]; total: number; page: number; limit: number; has_more: boolean; filters: Record<string, unknown> }>(`/api/admin/profiles/catalog/items${params}`, { token }),
